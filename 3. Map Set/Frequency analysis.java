...
public class Main {
     ...
     */
    public static ArrayList<HashMap.MapEntry<String, Integer[]>> sort(ArrayList<HashMap.MapEntry<String, Integer[]>> arr) {
...
//      Merge the sorted halves.
        ...
        while (lPointer < left.size() && rPointer < right.size()) {
            ...
            int rWordsCount = rEntry.getValue()[0];
            int lWordsCount = lEntry.getValue()[0];

            int rAppearanceIndex = rEntry.getValue()[1];
            int lAppearanceIndex = lEntry.getValue()[1];

            // Compare two entries based on the following criteria:
            // 1. Entries with a higher word count (rWordsCount, lWordsCount)
            // should come first (i.e., descending order).
            // 2. If the word counts are equal, the entry with a
            // lower appearance index (rAppearanceIndex, lAppearanceIndex)
            //    should come first (i.e., ascending order).
            if ((rWordsCount > lWordsCount) ||
                    (rWordsCount == lWordsCount && rAppearanceIndex < lAppearanceIndex)) {
                merged.add(rEntry);
                rPointer++;
            } else {
                merged.add(lEntry);
    ...
    /**
     * Reads input data, computes word frequencies along with their appearance indices,
     * sorts the words based on the frequency (in descending order) and appearance index (in ascending order),
     * and then prints the k-th word from the sorted order.
     *
     * <p>
     * For each word, the map stores an Integer array where:
     * <ul>
     *   <li>Index 0 holds the frequency (word count).</li>
     *   <li>Index 1 holds the word's appearance index (the position at which it last occurred).</li>
     * </ul>
     * </p>
     *
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (map.get(word) == null) {
                ...
                map.put(word, new Integer[]{1 + map.get(word)[0], i});
            }
        }

        ArrayList<HashMap.MapEntry<String, Integer[]>> wordsFreq = new ArrayList<>();
        for (Map.Entry<String, Integer[]> entry : map.entrySet()) {
            wordsFreq.add(new HashMap.MapEntry<>(entry.getKey(), entry.getValue()));
        }
        wordsFreq = sort(wordsFreq);
        System.out.println(wordsFreq.get(k - 1).getKey());
    }
